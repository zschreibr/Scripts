...
=head1 SYNOPSIS
    ...
    USAGE: batb_to_match.pl -i=btab file
...
=head1  DESCRIPTION
	Takes the ORFs in a btab file and counts how many ORFs with similar functional description are present within a range 

=head1  INPUT
	Btab file of called ORFs        

=head1  OUTPUT
        Tab file showing ORF distance and how many times an ORF with a similar functional description occurs 
...
print "ORF Position\t Similar Function Count\n";
		...
		if(exists $rHoH->{$ctgid}->{'POSITION'}[$i]){
			...
			$counter{$ab}++;
					...
					if(exists $rHoH->{$ctgid}->{'POSITION'}[$j]){
						if($rHoH->{$ctgid}->{'FUNCTION'}[$i] eq $rHoH->{$ctgid}->{'FUNCTION'}[$j]){
